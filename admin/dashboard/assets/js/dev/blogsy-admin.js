//--------------------------------------------------------------------//
// Blogsy script that handles our admin functionality.
//--------------------------------------------------------------------//
...
;( function( $ ) {
	...
	var BLOGSYAdmin = {
		...
		init: function() {
			...
			// Document ready
			$( document ).ready( BLOGSYAdmin.ready );

			// Window load
			$( window ).on( 'load', BLOGSYAdmin.load );

			// Bind UI actions
			BLOGSYAdmin.bindUIActions();

			// Trigger event when Blogsy fully loaded
			$( document ).trigger( 'blogsyReady' );
		},
		...
		*/
		bindUIActions: function() {
			...
			$wrap.on( 'click', '.plugins .hester-btn:not(.active)', function( e ) {
				...
				BLOGSYAdmin.pluginAction( $this );
			});

			$( document ).on( 'wp-plugin-install-success', BLOGSYAdmin.pluginInstallSuccess );
			$( document ).on( 'wp-plugin-install-error',   BLOGSYAdmin.pluginInstallError );
		},
		...
		pluginInstallSuccess: function( event, response ) {
			...
			BLOGSYAdmin.pluginAction( $button );
		},
	...
	BLOGSYAdmin.init();
	window.blogsyadmin = BLOGSYAdmin;
